...
class WcCommandTest < Test::Unit::TestCase
  FILENAME = ['./fixtures/Gemfile'].freeze
  FILENAMES = ['./fixtures/Gemfile', './fixtures/Gemfile.lock'].freeze
  TEXT_STDIN = <<~TEXT
    total 736
  ...
  def test_stdin
    expected = <<-TEXT.chomp
      22     191    1363
    TEXT
    assert_equal expected, WcCommand.display(text: TEXT_STDIN)
  end
  ...
  def test_stdin_with_loption
    expected = <<-TEXT.chomp
      22
    TEXT
    assert_equal expected, WcCommand.display(text: TEXT_STDIN, line: true)
  end
